...
export default async function ProductPage({ params }: Params) {
  ...
  const breadcrumb = breadcrumbJsonLd([
    { name: "Templates", url: `https://kdpcover.pro${STORE_PATH}` },
    { name: sku.name, url: `https://kdpcover.pro${STORE_PATH}/${sku.slug}` },
  ]);
    ...
    <article className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[1.4fr_1fr]">
        ...
        <aside className="lg:sticky lg:top-24 lg:h-fit">
          <div className="rounded-card border border-sage-200 bg-white p-6">
          ...
          <div className="mt-6 rounded-card border border-sage-200 bg-sage-50/50 p-5 text-xs text-sage-800">
            <p className="font-medium text-ink">Not sure which pack?</p>
            <p className="mt-1">
              Use the <Link href="/" className="underline hover:text-warm-500">free calculator</Link>{" "}
              to compute your exact spine width first — most authors find the Universal Pack covers
              every book they&rsquo;ll ever publish.
            </p>
          </div>
        </aside>
      </div>
