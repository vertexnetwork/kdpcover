...
export default async function SharePage({ params }: Params) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="mt-6 rounded-card border border-(--color-border) bg-(--color-surface) p-5 sm:p-6">
      ...
      <Link
        href={`/${reopenHash}`}
        className="mt-6 inline-flex items-center rounded-md bg-(--color-accent) px-4 py-2 text-sm font-medium text-(--color-on-accent) hover:opacity-90"
      >
        Open in calculator →
      </Link>
    </section>
