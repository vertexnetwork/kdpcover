  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <p className="mt-3 text-sage-800">
        KDP&rsquo;s reviewer checks geometry, bleed, safe zones, barcode placement, and file format. Every rejection we&rsquo;ve seen falls into one of seven buckets. Here they are, with the exact spec each one trips on.
      </p>
        ...
        <ul className="mt-3 space-y-2 text-sm text-sage-800">
          {CHECKLIST.map((c) => (
            <li key={c} className="flex items-start gap-2">
              <span aria-hidden className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-sage-400" />
              <span>{c}</span>
            </li>
          ))}
        </ul>
        ...
        {REJECTION_REASONS.map((r, i) => (
          <li key={r.title}>
            <h3 className="text-base font-medium">
              <span className="tabular text-sage-700">{String(i + 1).padStart(2, "0")}.</span> {r.title}
            </h3>
            <p className="mt-1 text-sm text-sage-800">{r.detail}</p>
          </li>
        ))}
      </ol>
      ...
      <p className="mt-12 rounded-card border border-sage-200 bg-sage-50 p-5 text-sm text-sage-800">
        Ready to compute your spine?{" "}
        <Link href="/" className="font-medium text-warm-500 hover:text-warm-700">
          Open the calculator →
        </Link>
      </p>
