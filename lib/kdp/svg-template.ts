...
export function buildTemplateSvg(input: CoverInput, output?: CoverCalcOutput): string {
  ...
  return [
    ...
    `<svg xmlns="http://www.w3.org/2000/svg" width="${round2(w)}pt" height="${round2(h)}pt" viewBox="0 0 ${round2(w)} ${round2(h)}">`,
    `  <title>KDP ${format} ${input.paper} ${input.pageCount}pp ${input.trimWidthIn}×${input.trimHeightIn} cover template</title>`,
    `  <desc>Spine ${out.spineWidthIn.toFixed(4)} in (${out.spineWidthMm.toFixed(2)} mm). Full cover ${out.fullCoverWidthIn.toFixed(4)} × ${out.fullCoverHeightIn.toFixed(4)} in. Generated by kdpcover.pro.</desc>`,
    ...layers.map((l) => `  ${l}`),
    `</svg>`,
  ].join("\n");
}
